...
#     X_train = X_train_fusion[i]
#     X_devel = X_devel_fusion[i]
for x in feat_fusion_3:
...
#                 X_test  = pd.read_csv(features_path + task_name + '.' + x + '.test.csv',  sep=sep, header=header, usecols=range(ind_off,num_feat+ind_off), dtype=np.float32).values
    ...
    uar_scores = []
    print(f'current features set is: {feat_fusion_3[i]}')
...
print(UAR)
print(sorted(zip(UAR,feat_fusion_3),reverse=True))